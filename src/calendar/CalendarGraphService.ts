...
const CONTACTID_FIELD_NAME =
  "String {4fa5d502-de1e-434e-9b07-cb3334aff457} Name ContactId";
...
export async function getUserCalendar(
  ...
  const response: PageCollection = await cachedGraphClient!
    ...
    .expand(
      `singleValueExtendedProperties($filter=id eq '${CONTACTID_FIELD_NAME}')`
    )
    .query({
...
export async function createEvent(
  ...
  const event = await cachedGraphClient!.api("/me/events").post(newEvent);
