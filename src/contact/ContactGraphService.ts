...
const SESSIONS_FIELD_NAME =
  "StringArray {d8fb8e2b-cc76-47f1-b95f-578b4b150bb6} Name Sessions";
...
export async function getContactList(
  ...
  const response: PageCollection = await cachedGraphClient!
    ...
    .select(
      "birthday,children,companyName,displayName,emailAddresses,givenName,surname,homeAddress,mobilePhone,personalNotes,profession,spouseName,photo"
    )
    .orderby("displayName")
    .top(5)
    .get();
...
export async function createContact(
  ...
  return await cachedGraphClient!.api("/me/contacts").post(newContact);
}

export async function updateArrayOfSessions(
  authProvider: AuthCodeMSALBrowserAuthenticationProvider,
  contact: Contact,
  sessions: string[]
): Promise<Contact> {
  ...
  await cachedGraphClient!.api(`/me/contacts/${contact.id}`).patch({
