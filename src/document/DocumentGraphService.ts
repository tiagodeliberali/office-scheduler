...
const buildDisplayName = (contact: Contact): string => contact.displayName!.substring(0, 50)

async function getOrCreateNotebooksSingleton(
    authProvider: AuthCodeMSALBrowserAuthenticationProvider): Promise<Notebook> {
...
async function createNotebook(authProvider: AuthCodeMSALBrowserAuthenticationProvider,
    notebook: Notebook): Promise<Notebook> {
    cachedGraphClient = ensureClient(authProvider, cachedGraphClient);
...
export async function initializeContactSection(
    ...
    const section = await loadOrCreateSection(authProvider, buildDisplayName(contact))
...
export async function addContactSession(
    ...
    const sessions = contact?.multiValueExtendedProperties && contact?.multiValueExtendedProperties.length > 0 && contact?.multiValueExtendedProperties[0].value || [];
    ...
    return await updateArrayOfSessions(authProvider, contact, [...sessions, session])
}